...
void Visualization::update_angles(AP_DATA2GCS_struct s) {
    if(_enabled) {
        _cuboidTransform->setRotationX(s.UAV_INS_Attitude[1]);
        _cuboidTransform->setRotationY(-s.UAV_INS_Attitude[0]);
        _cuboidTransform->setRotationZ(-s.UAV_INS_Attitude[2]);
    } else {
        _last = s;
        if(!_last_is_defined)
